...
function GetAllDataBase($database) {
	$sql = "SELECT * FROM operator
	         LEFT OUTER JOIN type_operator ON operator.type_id=type_operator.id 
	         LEFT OUTER JOIN result ON operator.id=result.id";
	$result  = array();
...
function AddData($database, $type_id, $operator, $name_operator){
...
function GetEditDB($database, $operator) {
...
function GetSearch($database){
	$sql = "SELECT * FROM operator
	        ...
	        INNER JOIN result ON operator.result_id=result.id WHERE 
	        operator.name_operator LIKE '%".$param."%' ";
